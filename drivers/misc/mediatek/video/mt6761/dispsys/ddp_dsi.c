...
void kdisp_debug_log_output_dsi_cmd_tx(char *bp, int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		memcpy(&logbuf[count], cmddata, 6);
		count += 6;
	}

//	KCDISP_LOG_SEQ("[KCDISP]%s  %s\n", __func__, logbuf);
	pr_notice("[KCDISP]%s  %s\n", __func__, logbuf);
}
#endif /* KCDISP_SEQ */
	...
	unsigned char force_update)
{
	...
	do {
...
#ifdef KCDISP_SEQ
		pr_notice("[KCDISP]%s: data_type=%x cmd=%x count=%d\n", __func__, data_id, cmd, count);
#endif /* KCDISP_SEQ */

		if (data_id == REGFLAG_ESCAPE_ID &&
			cmd == REGFLAG_DELAY_MS_V3) {
#ifdef KCDISP_CUST
			usleep_range(1000 * count, 1000 * (count+1));
#else
			udelay(1000 * count);
#endif /* KCDISP_CUST */
		...
		if (data_id == REGFLAG_ESCAPE_ID &&
			cmd == REGFLAG_HSCLK_EN_V3) {
				pr_err("[Disp]%s clk en\n",__func__);
				if(count > 0)
...
#ifdef KCDISP_SEQ
			kdisp_debug_log_output_dsi_cmd_tx((char *)para_list, count);
#endif /* KCDISP_SEQ */
...
#ifdef KCDISP_SEQ
			pr_err("[DSI]enter cmp check_tab cmd=0x%x,cnt=0x%x\n",
...
#ifdef KCDISP_SEQ
				pr_notice("[KCDISP] %s: buffer[%d]0x%x != lcm_esd_tb->para_list[%d]0x%x\n",
					__func__, j, buffer[j], j, lcm_esd_tb->para_list[j]);
#endif /* KCDISP_SEQ */
