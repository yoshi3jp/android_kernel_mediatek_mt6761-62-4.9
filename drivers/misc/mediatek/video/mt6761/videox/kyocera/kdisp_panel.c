...
extern void cat4004b_work(int level, enum kc_wled_table tbl, int gpio);
...
int kdisp_dsi_event_handler(struct mdss_panel_data *pdata, int event)
{
	...
	case KDISP_EVENT_PANEL_ON_POST1:
		pr_debug("[disp]%s: KDISP_EVENT_PANEL_ON_POST1\n", __func__);
		kdisp_dsi_panel_on_post1(pdata);
		break;
	case KDISP_EVENT_PANEL_ON_POST2:
		pr_debug("[disp]%s: KDISP_EVENT_PANEL_ON_POST2\n", __func__);
		kdisp_dsi_panel_on_post2(pdata);
		pr_debug("[disp]%s: KDISP_EVENT_PANEL_ON_POST2 END\n", __func__);
		msleep(1);
		break;
	default:
...
int kdisp_mipiic_dsi_panel_on_post(struct mdss_panel_data *pdata)
{
	...
	pr_debug("[kdisp]%son_post_cmds send\n", __func__);
	...
	int enable)
{
	...
	if (enable) {
		if (kctrl_data->mipi_vdd) {
			ret = regulator_set_load(kctrl_data->mipi_vdd, 10000);
			if (ret < 0) {
		...
		if (kctrl_data->mipi_vdd) {
			...
			ret = regulator_set_load(kctrl_data->mipi_vdd, 100);
			if (ret < 0) {
				pr_err("failed to set optimum mode mipi_vdd");
			}
		}
...
int kdisp_mipiic_reset_seq(struct kc_ctrl_info *kctrl_data, int enable)
{
	...
	if (enable) {
		...
		if (gpio_is_valid(kctrl_data->refclk_gpio)) {
			...
			pr_err("%s: [DISP]refclk ON\n", __func__);
		}
		usleep_range(3000, 4000);

		/*
		if (kctrl_data->dummy_post_cmds.cmd_cnt){
		mdss_dsi_panel_cmds_send(ctrl_pdata,
				(struct dsi_panel_cmds *)&kctrl_data->dummy_post_cmds, CMD_REQ_COMMIT);
		}
		*/
...
//	mdss_dsi_parse_dcs_cmds(np, (struct dsi_panel_cmds *)&kctrl_data->dummy_post_cmds,
//		"kc,mdss-dsi-dummy-command", "kc,mdss-dsi-dummy-command-state");
	...
	kctrl_data->vdd = regulator_get(&pdev->dev, "vdd");
	if(IS_ERR(kctrl_data->vdd)){
