...
void kdisp_debug_dump_mdp_timing_reg(char __iomem *base)
{
	...
	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_HSYNC_CTL);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_HSYNC_CTL=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_VSYNC_PERIOD_F0);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_VSYNC_PERIOD_F0=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_VSYNC_PULSE_WIDTH_F0);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_VSYNC_PULSE_WIDTH_F0=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_DISPLAY_HCTL);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_DISPLAY_HCTL=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_DISPLAY_V_START_F0);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_DISPLAY_V_START_F0=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_DISPLAY_V_END_F0);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_DISPLAY_V_END_F0=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_ACTIVE_HCTL);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_ACTIVE_HCTL=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_ACTIVE_V_START_F0);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_ACTIVE_V_START_F0=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_ACTIVE_V_END_F0);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_ACTIVE_V_END_F0=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_BORDER_COLOR);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_BORDER_COLOR=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_UNDERFLOW_COLOR);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_UNDERFLOW_COLOR=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_HSYNC_SKEW);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_HSYNC_SKEW=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_POLARITY_CTL);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_POLARITY_CTL=%x\n", __func__, reg);

	reg = readl_relaxed(base + MDSS_MDP_REG_INTF_FRAME_LINE_COUNT_EN);
	pr_err("%s: [DISP]MDSS_MDP_REG_INTF_FRAME_LINE_COUNT_EN=%x\n", __func__, reg);
}
