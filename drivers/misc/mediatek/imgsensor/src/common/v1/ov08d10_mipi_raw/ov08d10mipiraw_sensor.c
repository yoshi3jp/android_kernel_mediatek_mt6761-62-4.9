...
//Fac read/write
struct FAC_DATA_OV ov08d10_fac_data;
static int Fac_kobject_init = 0;
//Fac read/write				
#define EEPROM_WRITE_ID 0xAA						
...
static struct imgsensor_info_struct imgsensor_info = {						
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_R,	//FAE				
	.mclk = 24,					
...
{						
	...
	printk("--------I2C Read Start , ADDR 0x%x\n",addr);					
	iReadRegI2C(pu_send_cmd, 1, (u8*)&get_byte, 1, imgsensor.i2c_write_id);					
	printk("--------I2C Read End , ADDR 0x%x\n", get_byte);
	return get_byte;					
}						
...
{						
	...
	printk("--------I2C Write Start , ADDR 0x%x, PARA 0x%x\n",addr,para);		
	iWriteRegI2C(pu_send_cmd, 2, imgsensor.i2c_write_id);	
	printk("--------I2C Write End \n");				
}						
...
{			
	  ...
	  printk("huangchen enter %s",__func__);						
	  ...
	  printk("read ov08d10 version 0x0e = %d\n", ov08d10_version_value);					
...
{						
	...
	printk("huangchen enter %s",__func__);						
	printk("imgsensor.frame_length = %d\n", imgsensor.frame_length);					
...
{						
	kal_uint16  iReg;
    printk("huangchen enter %s",__func__);				
	if(gain >= 64) //base gain = 0x40					
	{					
		iReg = 16 * gain/BASEGAIN;        //change mtk gain base to aptina gain base				
		...
		{				
			...
			write_cmos_sensor(0xfd, 0x01);	//page1		
			write_cmos_sensor(0x01, 0x01);	//fresh		
			printk("PARKERA_SHINETECH_FRONT_OV08D10MIPI_SetGain = 16");			
		}				
		else if(iReg>= 251)//gpw				
		{				
			write_cmos_sensor(0xfd, 0x01);			
			write_cmos_sensor(0x24, 0xf8);			
			write_cmos_sensor(0xfd, 0x01);	//page1		
			write_cmos_sensor(0x01, 0x01);	//fresh		
			printk("PARKERA_SHINETECH_FRONT_OV08D10MIPI_SetGain = 160");			
		}				
		else				
		{				
			write_cmos_sensor(0xfd, 0x01);			
			...
			write_cmos_sensor(0xfd, 0x01);	//page1		
			write_cmos_sensor(0x01, 0x01);	//fresh		
			printk("PARKERA_SHINETECH_FRONT_OV08D10MIPI_SetGain = %d",iReg);			
		}				
	}					
	else					
...
static void set_mirror_flip(kal_uint8 image_mirror)						
{
	...
	printk("huangchen image_mirror = %d\n", image_mirror);					
}						
...
{						
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {					
		...
		*sensor_id = return_sensor_id();
		printk("huangchen ov08d10 PARKERA_SHINETECH_FRONT_OV08D10 get_imgsensor_id success: 0x%x\n", *sensor_id);		
		if (*sensor_id == imgsensor_info.sensor_id) {				
			if ((read_OV08D10_version()) < OV08D10_ECO_VALUE) {			
				*sensor_id = 0xFFFFFFFF;
				printk("lixu set OV08D10 0xffffffff");		
				return ERROR_NONE;		
			} else {			
				printk("PARKERA_SHINETECH_FRONT_OV08D10 get_imgsensor_id success: 0x%x\n", *sensor_id);		
				return ERROR_NONE;		
			}			
		}				
		retry--;				
	...
	if (*sensor_id != imgsensor_info.sensor_id) {					
		printk("lixu PARKERA_SHINETECH_FRONT_OV08D10 get_imgsensor_id failed: 0x%x\n", *sensor_id);				
		*sensor_id = 0xFFFFFFFF;				
		return ERROR_NONE;				
	}					
...
{						
	...
	printk("lixu mazhuang test enter");					
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {					
		...
		do {				
			sensor_id = return_sensor_id();			
			if (sensor_id == imgsensor_info.sensor_id) {			
				printk("lixu PARKERA_SHINETECH_FRONT_OV08D10 open success: 0x%x\n", sensor_id);		
				break;		
			}			
			retry--;			
		} while(retry > 0);				
...
{						
	printk("enable: %d\n", enable);					
	printk("huangchen enter %s %d",__func__,enable);						
	if(enable)					
	{					
...
{						
	...
	case SENSOR_FEATURE_SET_REGISTER:
	    printk("huangchen enter %s 0x%x",__func__,SENSOR_FEATURE_SET_REGISTER);						
	...
	case SENSOR_FEATURE_CHECK_SENSOR_ID:
	    printk("huangchen SENSOR_FEATURE_CHECK_SENSOR_ID=0x%x",SENSOR_FEATURE_CHECK_SENSOR_ID);					
...
//Fac read/write
static void ov08d10_fac_read(kal_uint32 addr,kal_uint16 *data)
{
	*data = read_cmos_sensor(addr);
}

static void ov08d10_fac_write(kal_uint32 addr,kal_uint32 para)
{
	write_cmos_sensor(addr,para);
}

ssize_t ov08d10_i2c_store(struct kobject *kobj,struct kobj_attribute *attr,const char *ubuf,size_t count)
{
	int i;
	...
	if(fac_ctx->size > OV_EEPROM_MAX_SIZE || fac_ctx->size <= 0)
		return -1;
	ov08d10_fac_data.size = fac_ctx->size;

	if(fac_ctx->flag == OV_SENSOR_I2C_READ){
		for(i = 0 ; i< fac_ctx->size ; i++){
			ov08d10_fac_read(fac_ctx->addr + i,&ov08d10_fac_data.data[i]);
		}
	} else if (fac_ctx->flag == OV_SENSOR_I2C_WRITE){
		ov08d10_fac_write(fac_ctx->addr,fac_ctx->data);
		mdelay(10);
		ov08d10_fac_read(fac_ctx->addr,&data);
	} else {
		return -1;
	}
...
ssize_t ov08d10_i2c_show(struct kobject *kobj,struct kobj_attribute *attr,char *ubuf)
{
	if(ov08d10_fac_data.size == 0)
		return -1;

	memcpy(ubuf,&ov08d10_fac_data,sizeof(struct FAC_DATA_OV));

	ov08d10_fac_data.size = 0;
	return sizeof(struct FAC_DATA_OV);
}

struct kobj_attribute i2c_kobj_attr_rdwr_ov08d10 = __ATTR(ov08d10_rdwr,0664,ov08d10_i2c_show,ov08d10_i2c_store);

struct attribute *OV08D10_RDWR_attr[] = {
	&i2c_kobj_attr_rdwr_ov08d10.attr,
	NULL,
};

struct attribute_group OV08D10_RDWR_attr_grp[] = {
	{
		.name = "fac_i2c_ov08d10",
		.attrs = OV08D10_RDWR_attr,
	},
	{},
};
...
{
	int ret;
	struct kobject *OV08D10_ReadWrite_kobj = NULL;

	OV08D10_ReadWrite_kobj = kobject_create_and_add("Fac_RDWR_ov08d10",NULL);
	if(OV08D10_ReadWrite_kobj == NULL)
		return;
	ret = sysfs_create_group(OV08D10_ReadWrite_kobj,&OV08D10_RDWR_attr_grp[0]);
	if(ret)
		kobject_put(OV08D10_ReadWrite_kobj);
